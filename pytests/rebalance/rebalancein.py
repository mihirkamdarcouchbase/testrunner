...
class RebalanceWithPillowFight(BaseTestCase):
    ...
    def load(self, server, items, batch=1000, docsize=100):
        ...
        cmd = "cbc-pillowfight -U couchbase://{0}/default -I {1} -m {4} -M {4} -B {2} -c 5000 --sequential --json -t {4} --rate-limit=100000" \
            .format(server.ip, items, batch, docsize, num_cores/2)
